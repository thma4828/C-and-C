...
public:
	...
	row * col_to_row(column *C) {
		...
		row *T = new row();
		for (int z = 0; z < int(C->col.size()); z++) {
	...
	int row_x_col(row *A, column *B) {
		if (A == NULL || B == NULL || A->r.size() != B->col.size()) {
